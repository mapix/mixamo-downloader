#!/usr/bin/env python3
"""
Mixamo Character Metadata Downloader V2 Improved
Improvement Strategy:
1. Download only Character type (excluding CharacterPack)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Mixamo 角色元数据下载器 V2",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  %(prog)s --output character_annotations
  %(prog)s -o /path/to/output/dir
    ...
    parser.add_argument(
        "-o", "--output",
        type=str,
        default="character_annotations",
        help="输出目录路径 (默认: character_annotations)"
    )
...
def fetch_characters_page(query="", page=1, limit=96):
    """
    Get character data for specified page (Character type only)
    ...
    params = {
        ...
        "limit": limit,
        "order": "",
        "type": "Character",  # Query Character type only
        "query": query
    }
...
def compare_characters(char1, char2):
    """
    Compare if two character data are completely identical
    ...
    # Get all keys
    all_keys = set(char1.keys()) | set(char2.keys())
...
def fetch_by_letter(letter):
    """
    Get all characters by alphabetic search

    Args:
        letter: Search letter

    Returns:
        list: All characters under this letter
    """
    characters = []
    page = 1
    total_pages = 1
    ...
    while page <= total_pages:
        page_data = fetch_characters_page(query=letter, page=page, limit=96)
...
def download_all_annotations(output_dir):
    """Download metadata for all characters"""
    print("=" * 70)
    print("Mixamo Character Metadata Downloader V2 Improved")
    ...
    # First try to query all Character directly
    print("\n[Step 1] Trying to query all Character directly...")
    first_page = fetch_characters_page(query="", page=1, limit=96)
    ...
    print(f"  - Total Character count: {total_results}")
    print(f"  - Total pages: {total_pages}")

    # Store all character data
    all_characters = {}  # id -> character_data
    ...
    # Alphabetic search
    print("\n[Step 2] Get characters by alphabetic search...")
    print(f"Target: Collect {total_results} unique characters")
    print("=" * 70)
    ...
    for idx, query in enumerate(search_queries, 1):
        # Check if enough unique characters have been collected
        if len(all_characters) >= total_results:
            print(f"\n✓ Collected {len(all_characters)} unique characters（reached total {total_results}）")
        ...
        if no_new_count >= max_no_new:
            print(f"\n⚠️  Consecutive {max_no_new} queries with no new characters, exiting early")
            print(f"Currently collected: {len(all_characters)}/{total_results} unique characters")
            break

        query_label = f"'{query}'" if query else "'(Other)'"
        progress = f"({len(all_characters)}/{total_results})"
        print(f"[{idx}/{len(search_queries)}] {progress} Search {query_label}...", end=" ")

        queries_executed += 1

        # Get all characters for this letter
        letter_characters = fetch_by_letter(query)

        new_count = 0
        duplicate_count = 0

        for character in letter_characters:
            ...
            duplicate_info[character_id].append({
                "query": query,
                "data": character
            })
            ...
            if character_id in all_characters:
                ...
                existing = all_characters[character_id]
                is_same, differences = compare_characters(existing, character)

                if not is_same:
                    different_duplicates.append({
                        "id": character_id,
                        "name": character.get("name"),
                        "queries": [d["query"] for d in duplicate_info[character_id]],
        ...
        letter_stats[query if query else "(Other)"] = {
            "fetched": len(letter_characters),
            "new": new_count,
            "duplicate": duplicate_count
        }

        print(f"Fetched: {len(letter_characters)}, New: {new_count}, Duplicate: {duplicate_count} ✓")
    ...
    print(f"\n[Step 3] Saving {len(all_characters)} unique characters...")
    saved_count = 0

    for character_id, character in all_characters.items():
        file_path = output_dir / f"{character_id}.json"
        try:
            with open(file_path, "w", encoding="utf-8") as f:
                json.dump(character, f, ensure_ascii=False, indent=2)
            saved_count += 1
        except Exception as e:
            print(f"\n✗ Saving {character_id} failed: {e}")
    ...
    print(f"\nData statistics:")
    print(f"  - Character total (API): {total_results}")
    print(f"  - Unique characters collected: {len(all_characters)}")
    print(f"  - Completeness: {len(all_characters)/total_results*100:.2f}%")

    if len(all_characters) >= total_results:
        print(f"  ✓ All characters collected!")
    elif len(all_characters) >= total_results * 0.99:
        print(f"  ✓ Collection nearly complete (missing {total_results - len(all_characters)} items)")
    else:
        print(f"  ⚠️  Possibly missing {total_results - len(all_characters)} characters")
    ...
    print(f"  - Average new per query: {len(all_characters)/queries_executed:.1f} items") if queries_executed > 0 else None

    print(f"\nRequest statistics:")
    print(f"  - Total entries returned: {total_fetched}")
    print(f"  - Duplicate ID count: {len(duplicate_ids)}")
    print(f"  - Total duplicate entries: {total_fetched - len(all_characters)}")
    print(f"  - Deduplication rate: {(1 - len(duplicate_ids)/len(all_characters))*100:.2f}%")
    ...
    if different_duplicates:
        ...
        diff_report_path = output_dir.parent / "character_duplicate_differences.json"
    ...
    summary = {
        ...
        "character_total": total_results,
        "unique_collected": len(all_characters),
        "completeness": len(all_characters) / total_results if total_results > 0 else 0,
        "queries_executed": queries_executed,
        "queries_total": len(search_queries),
        "query_coverage": queries_executed / len(search_queries) if len(search_queries) > 0 else 0,
        "avg_new_per_query": len(all_characters) / queries_executed if queries_executed > 0 else 0,
        "total_fetched": total_fetched,
        "unique_count": len(all_characters),
        "duplicate_count": len(duplicate_ids),
        "duplicate_rate": len(duplicate_ids) / len(all_characters) if len(all_characters) > 0 else 0,
        "deduplication_rate": (1 - len(duplicate_ids)/len(all_characters)) if len(all_characters) > 0 else 0,
    ...
    summary_path = output_dir.parent / "character_download_summary.json"
